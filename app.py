...
3. User Authentication & Role-Based Access (Flask-Login)
4. Inference engine (applying ML weights from model_parameters.csv)
"""
...
def load_model():
    """
    Loads the trained Logistic Regression parameters (weights, means, scales, and intercept)
    from the CSV file generated by model.py. This avoids the need to load large pickle 
    files, keeping the app lightweight and secure for cloud deployments.
    """
...
def run_prediction(payload):
    """
    Core Machine Learning Inference Function:
    1. Extracts biometric inputs from the payload.
    2. Reorders them to match the exact feature sequence used during training.
    3. Normalizes the data using the Z-score formula (value - mean) / scale.
    4. Computes the dot product of the normalized features and weights, plus the bias.
    5. Applies the Sigmoid function to output a probability (0.0 to 1.0).
    6. Logs the prediction in the database for tracking.
    """
